...
US_DATA_LRG_factors<-US_DATA_LRG %>% dplyr::select(c(PM2.5_UNC,SiteCode,Date))
  
## --- Do the same for the test set ---
US_DATA_LRG_PM_norm_test<-US_DATA_LRG_test %>%
dplyr::select(!c(PM2.5_UNC,SiteCode,Date)) %>% mutate(across(everything()),./ PM2.5) 
US_DATA_LRG_test_factors<-
## --- Save factor to merge back into DF ---
US_DATA_LRG_test_factors<-US_DATA_LRG_test %>% dplyr::select(c(PM2.5_UNC,SiteCode,Date))

## --- FInal output (I'm sure there's a cleaner way to do this) ---
US_DATA_LRG_PM_norm_test<-merge(US_DATA_LRG_test_factors,US_DATA_LRG_PM_norm_test)
...
US_PCA_DATA_slim_test<-as_tibble(dplyr::select(US_DATA_LRG_PM_norm_test,!contains(c("SiteCode","Date"))))
### --- log transform ---
...
ggplot(US_DATA_test_stack_GMM %>% dplyr::filter(GMM_ID == c(5)), aes(x = reorder(ind,values,FUN=median,na.rm=TRUE), y = values)) +
  geom_boxplot()+geom_line(data=med_sites,aes(x=species,y=medians,group=1))+ylab("Mass fraction")+xlab("Species")
