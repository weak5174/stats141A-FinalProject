...
### --- Step 3: Figures for the Porject report ---
```{r, "Figure 1: PM2.5 species boxplot: predictors in regressions, corrplot",warning=FALSE}
R<-cor(US_DATA_LRG %>% dplyr::select(!all_of(c("SiteCode","Date","PM2.5_UNC"))))
DZA<-corrplot(R,order="hclust")

## --- Boxplot on normal and log scale ---
F1A<-ggcorrplot(R,hc.order=TRUE)+theme(legend.position = "bottom")+theme(plot.title=element_text(hjust = 0.5,size=8),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size=6),axis.text.y = element_text(vjust = 0.5, hjust=1,size=6))+ggtitle("Sample Correlations: Predictors & response (=PM2.5)")
...
F1B<-ggplot(US_DATA_w_stack, aes(x = reorder(ind,values,FUN=median,na.rm=TRUE), y = values)) +
  geom_boxplot()+
  theme(plot.title=element_text(hjust = 0.5,size=8),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size=6))+ylab("Mass Concentration")+xlab("Species")+scale_y_log10(labels=scales::comma)+ggtitle("Summary of predictors sans response")

grid.arrange(D1A,D1B,nrow=1)
